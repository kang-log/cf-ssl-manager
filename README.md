...
**CF SSL Manager**是一款管理 CloudFlare 域名SSL证书的 Windows 桌面工具，通过CloudFlare API管理您的域名，轻松管理CloudFlare域名的SSL/TLS证书，自动申请免费SSL证书。，通过 CloudFlare API 管理你的域名，并使用 **ACME 协议**（Let's Encrypt）自动申请免费 SSL 证书。
